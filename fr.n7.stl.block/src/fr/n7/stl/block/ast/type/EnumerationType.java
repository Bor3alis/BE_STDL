...
public class EnumerationType implements Type, Declaration {
	...
	public boolean equalsTo(Type _other) {
		...
		if(_other instanceof EnumerationType) {	
			return this.name.equals(((EnumerationType)_other).name) && this.labels.equals(((EnumerationType)_other).labels);
		} else {
			return false;
		}
	}
	...
	public Type merge(Type _other) {
		if(_other.compatibleWith(this)){
			...
			if(this.compatibleWith(_other)) {
				...
				Logger.error("EnumerationType : typeMismatch");
	...
	@Override
	public int length() {
		int taille_liste = labels.size();
		int taille_label = labels.get(0).getType().length();
		return taille_liste * taille_label;
	}
